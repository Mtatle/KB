...
const discountsRewardsContent = {
    ...
    type: 'presentation',
    description: 'Training module on discounts, rewards programs, and promo codes',
    tags: ['training', 'module2', 'discounts', 'rewards', 'promo-codes'],
...
Ops Issues
The customer was told they'd receive a discount code but never got one, and no code is currently available
They reported not receiving their welcome code
The brand persona name is missing
An automated message sent contains an error
The Concierge Offer shown in the UI is invalid
...
Follow our workflow for finding discount codes
If none found, search in the #concierge-ops-issues channel to see if a code is available
...
2
If the code is not valid on our end and the items qualify: Post in the #concierge-ops-issues channel
3
Customer welcome code isn’t working
Has the customer already used the code on a previous order?
...
2
If the code is not valid, but was shared in an automated message: Post in the #concierge-ops-issues channel
3
...
But doesn’t mention the specific code or where they saw it, and we can’t determine it from the conversation
...
Let the customer know the code works, but has specific conditions (e.g. full-priced items only). Share any key exclusions and findings about the code you’ve observed
Politely confirm your findings:


Ask them to re-enter the code, making sure items meet the criteria and there are no typos or formatting issues
Suggest they try again:

Additional Troubleshooting

Customer Still Reports Issues
If the customer is still experiencing issues after you've provided clear guidance and they've tried again, it's time to gather more details
?
What to Do
What to Say
Kindly request a screenshot of their cart. This helps you see exactly what they’re experiencing and identify any issues that might not be obvious from their message
Thanks for giving that another try. If the code still isn’t working, could you please send a screenshot of your cart? It should show the items, the code entered, and any error message you’re seeing. That’ll help us better understand what’s happening on your side.
You can also use or adapt DER/DET templates 
...
if (window.documentRegistry) {
    window.documentRegistry.registerDocument(discountsRewardsContent, 'training', 'module2');
} 