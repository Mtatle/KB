...
# Generate HTML to test document accessibility
$testHtml = @"
<!DOCTYPE html>
<html>
<head>
...
foreach ($docId in $documentIds) {
    ...
    <div class="document-test">
        <h2>Test $index: ${docType} (${docId})</h2>
        <iframe id="iframe-$index" src="$embedUrl" onload="checkIframeLoaded(this, '$statusId')"></iframe>
        <div id="$statusId" class="status">Status: Testing...</div>
    </div>
"@
    $index++
}
